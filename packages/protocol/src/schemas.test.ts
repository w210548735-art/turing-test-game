...
describe("共享协议", () => {
  it("严格校验账户角色与 ROOT 后台统计契约", () => {
    expect(accountRoleSchema.safeParse("ROOT").success).toBe(true);
    expect(accountRoleSchema.safeParse("SUPER_ADMIN").success).toBe(false);
    expect(
  ...
  it("账户注销必须同时提供当前密码与明确确认文本", () => {
    expect(
      deleteAccountRequestSchema.safeParse({
        currentPassword: "Valid-Password-2026!",
        confirmation: "注销",
      }).success,
    ).toBe(true);
    expect(
    ...
    expect(
      deleteAccountResponseSchema.safeParse({ deleted: true }).success,
    ).toBe(true);
  });
